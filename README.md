...
- Implemented bulk addition using `contactList.addAll(contacts)` in the earlier design and evolved to counted insertion in the current version
...
When a contact with the same identity already exists in the selected address book, the new entry is rejected instead of being stored again.

#### Implementation Details

- Updated [AddressBook.java](/c:/Users/ASUS/OneDrive/Desktop/AddressBookApp/src/main/java/com/addressbookapp/model/AddressBook.java) so `addContact(Contact newContact)` now checks for duplicates before insertion
...
- Updated bulk insertion logic so only non-duplicate contacts are counted as successfully added
