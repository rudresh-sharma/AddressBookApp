...
The application starts with the core `Contact` model and expands through small, well-defined features such as adding contacts, editing details, deleting records, managing multiple contacts, organizing multiple address books, and eventually supporting search, sorting, file handling, and persistence.
...
- Updated contact operations to work against a named address book:
  - `addContact(String addressBookName, Contact contact)`
  - `addContacts(String addressBookName, List<Contact> contacts)`
  - `getContacts(String addressBookName)`
  - `editContact(String addressBookName, String firstName, Contact updatedContact)`
  - `deleteContact(String addressBookName, String firstName)`
