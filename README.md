...
- Extended [AddressBookService.java](/c:/Users/ASUS/OneDrive/Desktop/AddressBookApp/src/main/java/com/addressbookapp/service/AddressBookService.java) with `getContactsSortedByName(String addressBookName)` and `getAllContactsSortedByName()`
- Added a reusable `NAME_COMPARATOR` based on first name and last name using case-insensitive comparison
...
To sort contacts using city, state, or zip so that address book entries can be organized using location-based and postal-order views.

#### Functional Scope

UC12 extends sorting support beyond names.  
The feature allows contacts within a selected address book to be sorted by city, by state, and by zip, and the service also includes support for sorting all contacts across address books using the same criteria.
...
- Extended [AddressBookService.java](/c:/Users/ASUS/OneDrive/Desktop/AddressBookApp/src/main/java/com/addressbookapp/service/AddressBookService.java) with:
  - `getContactsSortedByCity(String addressBookName)`
  - `getContactsSortedByState(String addressBookName)`
  - `getContactsSortedByZip(String addressBookName)`
  - `getAllContactsSortedByCity()`
  - `getAllContactsSortedByState()`
  - `getAllContactsSortedByZip()`
- Added reusable comparators:
  - `CITY_COMPARATOR`
  - `STATE_COMPARATOR`
  - `ZIP_COMPARATOR`
- Implemented sorting with Java Stream API and comparator chaining
- Used first name and last name as secondary tie-breakers for stable ordering
- Returned an empty list when the requested address book does not exist

#### Test Coverage

- Added UC12 coverage in [AddressBookServiceTest.java](/c:/Users/ASUS/OneDrive/Desktop/AddressBookApp/src/test/java/com/addressbookapp/service/AddressBookServiceTest.java)
- Verified that contacts are sorted correctly by city
- Verified that contacts are sorted correctly by state
- Verified that contacts are sorted correctly by zip
- Confirmed the expected order for multiple inserted contacts

#### Outcome

UC12 successfully adds sorting by city, state, and zip, making it easier to inspect contacts from multiple organizational perspectives.
